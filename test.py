...
# Paste your dataset here as a single string
raw_data = """
...
# Regex patterns
game_pattern = re.compile(r"Game (\d+): (.*?)(?=Game \d+:|$)", re.DOTALL)
color_pattern = re.compile(r"(\d+)\s*(green|blue|red)")

games_data = []

for game_match in game_pattern.finditer(raw_data):
    game_number = int(game_match.group(1))
    draws_raw = game_match.group(2).strip()
    draws = [d.strip() for d in draws_raw.split(";")]
    
    for i, draw in enumerate(draws, start=1):
        colors = {"green": 0, "blue": 0, "red": 0}
        for num, color in color_pattern.findall(draw):
            colors[color] = int(num)
        games_data.append([game_number, i, colors["green"], colors["blue"], colors["red"]])

# Write to CSV
with open("games_dataset.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["Game", "Draw", "Green", "Blue", "Red"])
    writer.writerows(games_data)

print("CSV created with", len(games_data), "rows")