...
def fill_recursive(df):
    ...
    for index, row in df.iterrows():
        ...
        # Finally predict 2027 using 2020-2026 data (including the predictions for 2024, 2025, and 2026)
        row = recursive_regression(row, [2027], model)
...
Ny_df['2027'] = Ny_df['2027'].astype(int)
...
for index, row in Ny_df.iterrows():
    ...
    plt.plot([2023, 2024, 2025, 2026, 2027], row[['2023', '2024', '2025', '2026', '2027']], color=color, linestyle='--', marker='x')

plt.xlabel('Ar')
plt.ylabel('SUMMA matsvinn i kg')
plt.title('Forutspa framtidens matsvinn 2024-2027 baserad pa 2020-2023 data')
plt.legend(title="Manader", loc='center right')
plt.grid(True)
plt.show()

Ny_df.to_csv('Predictions 2024-2027.csv', index=False)